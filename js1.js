...
//display upto 8 movies
	...
	var card = card + "<div class='card grid-item'><img alt='' onerror='this.onerror=null; this.src=`no-image-found.jpg`' onclick='movie_modal("+JSON.stringify(movie_poster[j])+")' class='poster_img' style='max-width: 15vw;cursor: pointer;' src='"+poster_url+"'><h3 class='poster_title'>"+title+"</h3></div>";
...
//if > 8 movies, show showmore button and if clicked more movies, 90 more
	if(num > 8){
		...
		for(var j=8; j < num; j++){
			...
			var card2 = card2 + "<div class='card grid-item'><img alt='' onerror='this.onerror=null; this.src=`no-image-found.jpg`' onclick='movie_modal("+JSON.stringify(movie_poster[j])+")' style='max-width: 15vw;cursor: pointer;' src='"+poster_url+"'><h3 class='poster_title'>"+title+"</h3></div>";
